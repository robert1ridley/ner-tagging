...
class Viterbi(object):
  ...
  def calc_backpointers(self, observations, idx, item):
    ...
    if observations[idx - 2] not in self.word_tags_set:
      initial_observation = self.unique_tags
    else:
      initial_observation = self.word_tags_set[observations[idx - 2]]
    if observations[idx - 1] not in self.word_tags_set:
      second_observation = self.unique_tags
    else:
      second_observation = self.word_tags_set[observations[idx - 1]]
    max_prob = float('-inf')
    pointer = "*"
    for second_obs_tag in second_observation:
      for initial_obs_tag in initial_observation:
        probability = 0.0
        if (initial_obs_tag, second_obs_tag, item) in self.transition_probabilities:
          transition_prob = self.transition_probabilities[(initial_obs_tag, second_obs_tag, item)]
        else:
          if (initial_obs_tag, second_obs_tag) in self.bigram_counts:
            transition_prob = math.log(
              1.0 / float(self.bigram_counts[(initial_obs_tag, second_obs_tag)] + self.no_tags)
            )
          else:
            transition_prob = math.log(1.0 / float(self.no_tags))
        if (observations[idx], item) not in self.emission_probabilities:
          transition_prob = 0.0
          probability = self.calc_backpointers(observations, idx - 1, second_obs_tag) + transition_prob
        else:
          probability = self.calc_backpointers(observations, idx - 1, second_obs_tag) + \
                        self.emission_probabilities[(observations[idx], item)] + transition_prob
        if max_prob < probability:
          max_prob = probability
          pointer = second_obs_tag
